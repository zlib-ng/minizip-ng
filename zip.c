...
local int add_data_in_datablock(linkedlist_data *ll, const void *buf, uint32_t len)
{
    linkedlist_datablock_internal *ldi = NULL;
...
local int zipWriteValue(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream,
...
local uint64_t zipReadValueFromMemory(void* src, uint32_t len)
{
...
local void zipWriteValueToMemory(void* dest, uint64_t x, uint32_t len)
{
...
local int zipReadUInt16(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream, uint16_t *value)
{
    uint16_t x = 0;
    uint8_t c = 0;
    int err = ZIP_OK;
...
local int zipReadUInt32(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream, uint32_t *value)
{
    uint32_t x = 0;
    uint8_t c = 0;
    int err = ZIP_OK;
...
local int zipReadUInt64(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream, uint64_t *value)
{
    uint64_t x = 0;
    uint8_t c = 0;
    int err = ZIP_OK;
...
local int zipGetDiskSizeAvailable(zipFile file, uint64_t *size_available)
{
...
local int zipGoToSpecificDisk(zipFile file, uint32_t number_disk, int open_existing)
{
    zip64_internal *zi = NULL;
    int err = ZIP_OK;
...
local uint64_t zipSearchCentralDir(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream)
{
...
local uint64_t zipSearchCentralDir64(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream,
    ...
    int strategy, const char *password, uint32_t crc_for_crypting, uint16_t version_madeby, uint16_t flag_base, int zip64)
{
    ...
    zipWriteValueToMemory(zi->ci.central_header, (uint32_t)CENTRALHEADERMAGIC, 4);
    zipWriteValueToMemory(zi->ci.central_header + 4, version_madeby, 2);
    zipWriteValueToMemory(zi->ci.central_header + 6, (uint16_t)20, 2);
    zipWriteValueToMemory(zi->ci.central_header + 8, zi->ci.flag, 2);
    zipWriteValueToMemory(zi->ci.central_header + 10, zi->ci.method, 2);
    zipWriteValueToMemory(zi->ci.central_header + 12, zi->ci.dos_date, 4);
    zipWriteValueToMemory(zi->ci.central_header + 16, (uint32_t)0, 4); /*crc*/
    zipWriteValueToMemory(zi->ci.central_header + 20, (uint32_t)0, 4); /*compr size*/
    zipWriteValueToMemory(zi->ci.central_header + 24, (uint32_t)0, 4); /*uncompr size*/
    zipWriteValueToMemory(zi->ci.central_header + 28, size_filename, 2);
    zipWriteValueToMemory(zi->ci.central_header + 30, size_extrafield_global, 2);
    zipWriteValueToMemory(zi->ci.central_header + 32, size_comment, 2);
    zipWriteValueToMemory(zi->ci.central_header + 34, (uint16_t)zi->ci.number_disk, 2); /*disk nm start*/

    if (zipfi == NULL)
        zipWriteValueToMemory(zi->ci.central_header + 36, (uint16_t)0, 2);
    else
        zipWriteValueToMemory(zi->ci.central_header + 36, zipfi->internal_fa, 2);
    if (zipfi == NULL)
        zipWriteValueToMemory(zi->ci.central_header + 38, (uint32_t)0, 4);
    else
        zipWriteValueToMemory(zi->ci.central_header + 38, zipfi->external_fa, 4);
    if (zi->ci.pos_local_header >= 0xffffffff)
        zipWriteValueToMemory(zi->ci.central_header + 42, UINT32_MAX, 4);
    else
        zipWriteValueToMemory(zi->ci.central_header + 42,
    ...
    {
        if (zi->ci.zip64)
            err = zipWriteValue(&zi->z_filefunc, zi->filestream, UINT32_MAX, 4); /* compressed size, unknown */
        else
    ...
    if (err == ZIP_OK)
    {
        if (zi->ci.zip64) /* uncompressed size, unknown */
            ...
            err = zipWriteValue(&zi->z_filefunc, zi->filestream, (uint32_t)0, 4);
    }
    if (err == ZIP_OK)
    ...
    {
        uint16_t extra_header_id = 1;
        uint16_t extra_data_size = 16;
        ...
        err = zipWriteValue(&zi->z_filefunc, zi->filestream, extra_header_id, 2);
        err = zipWriteValue(&zi->z_filefunc, zi->filestream, extra_data_size, 2);

        err = zipWriteValue(&zi->z_filefunc, zi->filestream, uncompressed_size, 8);
        err = zipWriteValue(&zi->z_filefunc, zi->filestream, compressed_size, 8);
    }
#ifdef HAVE_AES
    ...
    if ((err == Z_OK) && (password != NULL))
    {
...
#endif
        {
            ...
            zi->ci.crypt_header_size = size_head;
...
local int zipFlushWriteBuffer(zip64_internal *zi)
{
    uint64_t size_available = 0;
    uint32_t written = 0;
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file, uint64_t uncompressed_size, uint32_t crc32)
{
    ...
    if (compressed_size >= UINT32_MAX || uncompressed_size >= UINT32_MAX || zi->ci.pos_local_header >= UINT32_MAX)
    {
        /* Do not change "version made by" upper byte since it might have been set by the user */
        uint16_t version_madeby = (uint16_t)zipReadValueFromMemory(zi->ci.central_header + 4, 2); /* version made by */
        if ((version_madeby & 0xff) < 45)
            zipWriteValueToMemory(zi->ci.central_header + 4, (uint16_t)((version_madeby & 0xff00) | (uint32_t)45), 2); /* version made by */
        zipWriteValueToMemory(zi->ci.central_header + 6, (uint16_t)45, 2); /* version needed */
    }
    zipWriteValueToMemory(zi->ci.central_header + 16, crc32, 4); /* crc */
    if (compressed_size >= UINT32_MAX)
        zipWriteValueToMemory(zi->ci.central_header + 20, invalid_value, 4); /* compr size */
    else
        zipWriteValueToMemory(zi->ci.central_header + 20, compressed_size, 4); /* compr size */
    ...
    if (uncompressed_size >= UINT32_MAX)
        zipWriteValueToMemory(zi->ci.central_header + 24, invalid_value, 4); /* uncompr size */
    else
    ...
    if (extra_data_size > 0)
    {
        ...
        zipWriteValueToMemory(p, 0x0001, 2);
        p += 2;
        zipWriteValueToMemory(p, extra_data_size, 2);
        p += 2;

        if (uncompressed_size >= UINT32_MAX)
        {
            zipWriteValueToMemory(p, uncompressed_size, 8);
            p += 8;
        }
        if (compressed_size >= UINT32_MAX)
        {
            zipWriteValueToMemory(p, compressed_size, 8);
            p += 8;
        }
        if (zi->ci.pos_local_header >= UINT32_MAX)
        {
            zipWriteValueToMemory(p, zi->ci.pos_local_header, 8);
            p += 8;
        }
    ...
    if (zi->ci.method == AES_METHOD)
    {
        ...
        zipWriteValueToMemory(p, 0x9901, 2);
        p += 2;
        zipWriteValueToMemory(p, extra_data_size, 2);
        p += 2;
        zipWriteValueToMemory(p, AES_VERSION, 2);
        p += 2;
        zipWriteValueToMemory(p, 'A', 1);
        p += 1;
        zipWriteValueToMemory(p, 'E', 1);
        p += 1;
        zipWriteValueToMemory(p, AES_ENCRYPTIONMODE, 1);
        p += 1;
        zipWriteValueToMemory(p, zi->ci.compression_method, 2);
        p += 2;
    ...
    if (err == ZIP_OK)
    {
        ...
        uint32_t cur_number_disk = zi->number_disk;
        ...
        if (ZSEEK64(zi->z_filefunc, zi->filestream, zi->ci.pos_local_header + 14, ZLIB_FILEFUNC_SEEK_SET) != 0)
            err = ZIP_ERRNO;
        if (err == ZIP_OK)
            err = zipWriteValue(&zi->z_filefunc, zi->filestream, crc32, 4); /* crc 32, unknown */

        if (uncompressed_size >= UINT32_MAX || compressed_size >= UINT32_MAX)
        {
        ...
        {
          if (err == ZIP_OK) /* compressed size, unknown */
              err = zipWriteValue(&zi->z_filefunc, zi->filestream,compressed_size, 4);
          if (err == ZIP_OK) /* uncompressed size, unknown */
              err = zipWriteValue(&zi->z_filefunc, zi->filestream,uncompressed_size, 4);
        }
