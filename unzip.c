    ...
    if ((err==UNZ_OK) && (file_info.size_file_extra != 0))
    {
        ...
        while((err!=UNZ_ERRNO) && (acc < file_info.size_file_extra))
        {
            ...
            else if (headerId == 0x9901)
            {
                ...
                /* Verify version info */
                if (unz64local_getShort(&s->z_filefunc, s->filestream_with_CD,&i) != UNZ_OK)
                    err=UNZ_ERRNO;
                if (i != 1)
                    err = UNZ_ERRNO;
                if (unz64local_getShort(&s->z_filefunc, s->filestream_with_CD,&i) != UNZ_OK)
                    err=UNZ_ERRNO;
                if (((char)(i) != 'A' || (char)(i >> 8) != 'E'))
                    err=UNZ_ERRNO;
                                            ...
                                            int* level, int raw, const char* password)
{
    ...
    if (!raw)
    {
        ...
        {
            pfile_in_zip_read_info->stream.zalloc = (alloc_func)alloc_func_uzimpl;
            pfile_in_zip_read_info->stream.zfree = (free_func)free_func_uzimpl;
